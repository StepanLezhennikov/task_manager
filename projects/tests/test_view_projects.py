...
def test_create_project_user_for_user(api_client, project_user_for_project_data):
    ...
    response = api_client.post(user_url, data=project_user_for_project_data, format='json')
    assert response.status_code == status.HTTP_201_CREATED
...
def test_create_project_user_with_invalid_data(api_client, project_user_for_project_data_invalid):
    ...
    user_url = f"/api/project_users/{project_id}/add-user-to-project/"
    response = api_client.post(user_url, data=project_user_for_project_data_invalid, format='json')
    assert response.status_code == status.HTTP_400_BAD_REQUEST
